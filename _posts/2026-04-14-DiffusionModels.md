...
$$\mathbb{E}_{q(x_{1:T} \mid x_{0})}\left[\underbrace{D_{KL}(q(x_{T} \mid x_{0}) \mid\mid p(x_{T}))}_{L_{T}} + \sum_{t > 1}  \underbrace{D_{KL}(q(x_{t-1}\mid x_{t},x_{0})\mid \mid p_{\theta}(x_{t-1} \mid x_{t}))}_{L_{t-1}}\; \underbrace{- \log\left( p_{\theta}(x_{0} \mid x_{1}) \right)}_{L_{0}}\right]$$ Here $L_{T}$ is essentially constant because $p(x_{T}) =\mathcal{N}(0,I)$ and doesn't depend on the parameters $\theta$.Therefore we can do no gradient updates.
